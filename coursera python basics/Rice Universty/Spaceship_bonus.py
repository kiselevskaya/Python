...
rock_info = ImageInfo([64, 64], [128, 128], 64, 64, True)
rock_image = simplegui.load_image("http://commondatastorage.googleapis.com/codeskulptor-assets/asteroid1.opengameart.warspawn.png")
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            image_index = (self.age % self.lifespan) // 1
            self.image_center = [self.image_center[0] + (image_index * self.image_size[0]), self.image_size[1]]
            canvas.draw_image(self.image, self.image_center, self.image_size, self.pos, self.image_size, self.angle)
        else:
            canvas.draw_image(self.image, self.image_center, self.image_size, self.pos, self.image_size, self.angle)
...
def group_collide(group, other_object):
    ...
    for element in set(group):
        if element.collide(other_object):
            new_explosion = Sprite(element.pos, [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
...
def rock_spawner():
    ...
    #a_rock = Sprite(rock_pos, rock_vel, 0, rock_angvel, rock_image, rock_info)
    if started:
        if dist(a_rock.pos, my_ship.pos) > (a_rock.radius + my_ship.radius) * 1.1:
            if len(rock_group) < 12:
                rock_group.add(a_rock)
    if score > 0:
        rock_vel = [rock_vel[0] * math.ceil(float(score)) / math.factorial(score),
            rock_vel[1] * math.ceil(float(score)) / math.factorial(score),]
...
missile_group = set([])
explosion_group = set([])
#new_explosion = Sprite([WIDTH / 3, HEIGHT / 3], [0,0], 0, 0, explosion_image, explosion_info, explosion_sound)
